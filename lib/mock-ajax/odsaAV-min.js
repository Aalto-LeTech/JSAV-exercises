 ...
 * ------------------------------------------------------------------------- */
//
// This extension to OpenDSA lib/odsaAV.js would make a JSAV exercise
// submission to mooc-grader (part of A+ LMS).
// However, in the context of the testbench of JSAV-exercises repository, the
// actual submission code has been removed.
//
...
(function($) {
  ...
  // Mimicks a HTTP POST request to mooc-grader.
  function sendSubmission(exerciseRecording) {
    ...
    if (typeof(exerciseRecording) !== "string") {
      ...
      // Assume stringified JSON (the standard JSAV way). Exclude characters
      // which are not in the ASCII character set (everything Unicode).
      sendableRecording = exerciseRecording.replace(/[^\x00-\x7F]/g, "")
    }
    // Pretend to send the data to A+; AJAX request always succeeds here.
  ...
  // Generates a grade text that can be displayed to the user.
