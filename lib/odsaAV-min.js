...
//*****************************************************************************
//*************             A+ LMS SPECIFIC CODE                  *************
//*************           Used at Aalto University                *************
//*****************************************************************************

// The MD5 checksum algorithm.
// This can be called by `md5(data)` from the namespace of this file.
! function(n) {
  "use strict";
  ...
  function l(n, t) {
    var r, e, o = h(n),
      u = [],
      c = [];
    for (u[15] = c[15] = void 0, o.length > 16 && (o = i(o, 8 * n.length)),
         r = 0; 16 > r; r += 1) u[r] = 909522486 ^ o[r], c[r] = 1549556828 ^ o[r];
...
(function(e) {
  ...
  var n = JSAV._types.Exercise.prototype;
  var messageToUser = "";
  ...
  //     return String.fromCharCode(e - 7)
  //   }).join("")
  // }([123, 62, 61, 120, 60, 59, 78, 125]);

  var s = JSAV.ext.exercise;
  JSAV.ext.exercise = function(t, a, r) {
    r = $.extend(r, {
      feedback: "atend"
    });
    return s.call(this, t, a, r)
  };
  $("#help").remove();
  $("#about").remove();

  $.extend(true, JSAV._translations, ajaxResultMessages);

  function jsonDumpEscaped(e) {
    return e._jsondump().replace(/[^\x00-\x7F]/g, "")
  }

  n.originalModel = n.showModelanswer;
  n.originalReset = n.reset;

  n.showModelanswer = function() {
    this.modelSeenFlag = true;
    e('.jsavexercisecontrols input[name="grade"]').attr("disabled", "disabled");
    this.originalModel()
  };
  ...
  n.reset = function() {
    ...
    e('.jsavexercisecontrols input[name="grade"]').removeAttr("disabled")
  };
  ...
  n.showGrade = function() {
    ...
    this.jsav.logEvent({
      ...
      score: e.extend({}, this.score)
    });
    translator = this.jsav._translate;
    var exerciseRecording = jsonDumpEscaped(this);
    var r = this.score;
    var n = [PARAMS.submission_url, exerciseRecording, r.correct, r.total].join(":");

    messageToUser = translator("yourScore") + " " + r.correct + " / " + r.total + "\n\n";

    // MODIFIED FOR TRAKY
    // Round (Math.floor) jsav points in exercise grading feedback modal
    // Aaro Korhonen 2017
    if (PARAMS['max_points']) {
      const scale = parseInt(PARAMS['max_points'], 10) / r.total;
      messageToUser = translator("yourScore") + " " + Math.floor(r.correct * scale) + " / " +
          Math.floor(r.total * scale) + "\n\n";
    }

    console.log("DEBUG from odsaAV-min.js: PARAMS.submit_url = " + PARAMS.submit_url)
    ...
    $.ajax(PARAMS.submit_url, {
        type: "POST",
        data: {
          checksum: md5(PARAMS.ajax_key + n),
          submission_url: PARAMS.submission_url,
          answer: exerciseRecording,
          points: this.score.correct,
          max_points: this.score.total,
  ...
  function ajaxReceived(response) {
    console.log("DEBUG from odsaAV-min.js: received from mooc-grader: " + response)
    if (response.success) {
