...
(function(e) {
  ...
  exerciseProto.showGrade = function() {
    ...
    // exercise as recording and continue to sendSubmission().
      ...
      const recording = exercise._jsondump().replace(/[^\x00-\x7F]/g, "");
  ...
  function sendSubmission(exerciseRecording) {
    ...
    let sendableRecording = "";
    if (typeof(exerciseRecording) !== "string") {

      // Assume that JSAV Exercise Recorder is in use.
      // Encode in base64 to ensure that the data will not break even if it
      // goes through Django template processor on the server.
      sendableRecording = btoa(JSON.stringify(exerciseRecording))
    }
    else {
