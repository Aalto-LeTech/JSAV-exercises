...
(function(e) {
  ...
  //          exercise.
  function getExerciseRecording(exercise) {
    ...
    try {
      let recording = JSAVrecorder.getRecording();
  ...
  n.showGrade = function() {
    ...
    var exerciseRecording = getExerciseRecording(this);

    messageToUser = gradeText(this.score.correct, this.score.total)
  ...
  function gradeText(correct, total) {
    ...
    if (PARAMS['max_points']) {
      const scale = parseInt(PARAMS['max_points'], 10) / total;
    ...
    return translator("yourScore") + " " + dispCorrect + " / " + dispTotal +
        "\n\n";
  }
