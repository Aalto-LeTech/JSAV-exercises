...
(function() {
  ...
  */
  ...
  function init() {
    ...
    // 2) while executing Kruskal's algorithm, the number of discarded edges before
    // the MST is complete is at least MINIMUM_DISCARDS.

    // Helper function that checks if the graph is connected.
    function isConnected(unionCount) {
      // The graph is connected if the number of unions made while executing
      // Kruskal's algorithm is equal to the number of vertices - 1.
      return unionCount === nVertices - 1;
    }
    ...
    while (trials < 150) {
      ...
      const {unionCount, discardCount} = countKruskalOperations(nlGraph, trials);

      debugPrint(`Trial ${trials}: unionCount: ${unionCount} discardCount: ${discardCount}`);

      if (isConnected(unionCount) && discardCount >= MINIMUM_DISCARDS) {
        // Suitable graph found.
        break;
      }
    }
 ...
 */
  ...
  /**
   * Executes Kruskal's algorithm on the given graph and counts the number of added
   * and discarded edges before the MST is complete.
   * @param {object} nlGraph. Graph in adjacency list format as generated by graphUtils.generatePlanarNl.
   * @param {number} trials - Number of trials in generating the graph so far. Is used for debug printing.
   * @returns {{unionCount: number, discardCount: number}} - Object with the number of unions and discards
   */
  function countKruskalOperations(nlGraph, trials) {
    // Helper function that takes an array of operations as strings and counts
    // the number of unions and discards before the MST is complete.
    function countOperations(operationsArr) {
      // First count unions and find the index of the last union operation.
      let unionCount = 0;
      ...
      return {unionCount, discardCount};
    }
    ...
    // Create array that stores the operations (e.g. "union A-B" or "discard A-B")
    // as strings.
    const operations = [];
    ...
    return countOperations(operations);
  }
